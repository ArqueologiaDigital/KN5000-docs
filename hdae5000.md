...
| 0x003C2-0x0F575 | 0x2803C2 | 61,876B | Code section 1B - Register_Frame and utilities |
| 0x0F576-0x0F661 | 0x28F576 | 236B | Boot initialization routine |
| 0x0F662-0x0F784 | 0x28F662 | 291B | Frame handler main loop |
| 0x0F785-0x0F8DF | 0x28F785 | 347B | Clear_Work_Buffer and utility routines |
| 0x0F8E0-0x0F90A | 0x28F8E0 | 43B | Load_Palette (256 VGA entries) |
...
| 0x15642-0x6FFFF | 0x295642 | 436,670B | Code section 2B - command handlers and data |
...
| 0x28F785 | HDAE5000_Clear_Work_Buffer | Clear 0xF52A bytes at 0x22A000, copy init data |
| 0x28F7DD | HDAE5000_Delay_Loop | Delay/timing utility loop |
| 0x28F7EE | HDAE5000_VGA_Port_Write | Write to VGA DAC ports 0x3C8/0x3C9 |
| 0x28F813 | HDAE5000_Palette_Setup | Configure single palette entry |
| 0x28F8E0 | HDAE5000_Load_Palette | Load all 256 palette entries |
...
The HDAE5000 uses VGA DAC registers for palette control:
- Port `0x3C8`: Palette index (write)
- Port `0x3C9`: Palette data (R, G, B sequentially)

`HDAE5000_Load_Palette` at `0x28F8E0` loads 256 entries from ROM data, calling `HDAE5000_Palette_Setup` at `0x28F813` for each entry.
...
| Boot initialization | **Disassembled** | 236 bytes at 0x28F576, fully disassembled with labels |
| Frame handler | **Labeled** | Split into 4 sections with 4 exposed labels |
| PPORT commands | **100%** | All 21 command strings + 12 handler addresses documented |
| PPORT jump table | **Disassembled** | 12 labeled dd entries with EQU symbols |
| PPORT strings | Binary | `pport_strings_295412_295641.bin` (560 bytes) |
| Embedded images | **100%** | 4 images + 2 palettes extracted (~233KB) |
| HD routines | Partial | Key addresses identified |

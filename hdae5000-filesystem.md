...
|-----------|---------|------|------|
| HDAE5000_FS_Read_FSB | 0x287F55 | 832 bytes | Display UI initialization (reads from RAM, not disk) |
| HDAE5000_FS_Write_FSB | 0x288295 | 5,072 bytes | Serializes in-RAM state to disk sectors |
| HDAE5000_Cmd03_ReadFSB | 0x2959F6 | 838 bytes | Reads FSB sectors from disk (PPORT interface) |
| HDAE5000_Cmd06_WriteFSB | 0x296294 | 150 bytes | Writes FSB sectors to disk (PPORT interface) |
...
`HDAE5000_FS_Write_FSB` serializes the FSB across multiple sectors:
...
HDAE5000_FS_Read_FSB initializes 24 entries from a ROM template at 0x2E2E60:
...
HDAE5000_FS_Write_FSB serializes entries using a 37-byte format from ROM template at 0x2E2EC0:
...
FGBs function as directory-like containers, grouping related files together within a partition. Their exact on-disk layout is embedded within the sector data read by `HDAE5000_Cmd03_ReadFSB`, which uses 9 region descriptors to address different parts of the filesystem. Each region descriptor is a 32-bit sector address.

### FEB (File Entry Block)

FEBs contain individual file metadata for each stored file on the disk. They are accessed through `HDAE5000_FS_Entry_Lookup` (0x28A2F0) using the 9-byte directory entry format described below.
...
Each partition maintains up to 40 directory entries at RAM 0x230884 (360 bytes total). Entries are insertion-sorted by name. Each 9-byte entry contains handler indices used by `HDAE5000_FS_Entry_Lookup` to dispatch through the function pointer table at ROM 0x2E1E14:
...
| 0x229DAD | byte | Content type 1 (for HDAE5000_File_Save type code dispatch) |
| 0x229DAE | byte | Content type 2 (for HDAE5000_File_Save type code dispatch) |
...
| 0x22AA4E-0x22AA57 | 10 bytes | Spinbox state bytes (one per event handler sub-routine in HDAE5000_FS_Init) |
...
| 0x23085E | word | Bytes consumed by last VarInt decode (set by HDAE5000_Calc_Disk_Space) |

### File Operation State (0x2304xx)

| Address | Size | Purpose | Routines |
|---------|------|---------|----------|
| 0x230430 | word | Current file type code (from sector table) | HDAE5000_File_Format, HDAE5000_File_Rename |
| 0x230432 | word | File operation flags | HDAE5000_File_Load |
| 0x230434 | word | Abort flag (1 = type 0x2F encountered) | HDAE5000_File_Format, HDAE5000_File_Delete |
| 0x230436 | word | File length (sectors) | HDAE5000_File_Format, HDAE5000_File_Load |
| 0x230438 | word | Start sector | HDAE5000_File_Format, HDAE5000_File_Save |
| 0x23043A | word | End position (start + allocation) | HDAE5000_File_Format |
| 0x23043C | word | Backup: previous start sector | HDAE5000_File_Format (backup mode) |
| 0x230440 | dword | Cumulative free space | HDAE5000_Calc_Disk_Space, HDAE5000_File_Format |
| 0x230444 | dword | Total allocation (free + remaining) | HDAE5000_File_Format, HDAE5000_File_Delete |
| 0x230448 | dword | Backup: previous free space | HDAE5000_File_Format (backup mode) |
| 0x23044C | dword | Backup: previous sector count | HDAE5000_File_Format (backup mode) |
| 0x230450 | dword | Remaining free space (after allocation) | HDAE5000_File_Format |
| 0x230454 | dword | Backup: previous remaining space | HDAE5000_File_Format (backup mode) |
| 0x230458-0x2304D7 | 128 bytes | Filename buffer 1 (null-terminated, max 127 chars) | HDAE5000_File_Format |
| 0x2304D8-0x2304E7 | 16 bytes | String lengths per directory slot (at offset slot+16) | HDAE5000_File_Delete |
| 0x2304E0-0x2304EF | ~16 bytes | File save progress state | HDAE5000_File_Save |
| 0x2304F0 | byte | Additional file metadata flag | HDAE5000_File_Load |

### Filename & Display Buffers (0x2306xx-0x2307xx)

| Address | Size | Purpose | Routines |
|---------|------|---------|----------|
| 0x230636-0x2306B5 | 128 bytes | Filename buffer 2 (backup/secondary, max 127 chars) | HDAE5000_File_Format, HDAE5000_File_Load |
| 0x2306B6 | ~46 bytes | Filename destination (from ROM template) | HDAE5000_File_Save |
| 0x230736 | ~50 bytes | Loaded filename slot 1 (max 50 chars) | HDAE5000_File_Load |
| 0x230768 | ~40 bytes | Loaded filename slot 2 (max 40 chars) | HDAE5000_File_Load |
| 0x230790 | ~46 bytes | Format string buffer (audio params display) | HDAE5000_File_Load |

### Audio Settings (from HDAE5000_File_Load)
...
| 0x2307AA | word | File params flag 1 | 1 (set by HDAE5000_File_Save) |
| 0x2307AC | word | File params flag 2 | 0 (set by HDAE5000_File_Save) |
| 0x2307AE | word | Samples per channel | 24/12/6/3 |
| 0x2307B0 | word | Samples per channel (copy) | 24/12/6/3 |
| 0x2307B6 | word | Error code (from HDAE5000_File_Format) | 0xFFFF-0xFFFA |

### Sector Metadata (0x2308xx)

| Address | Size | Purpose | Routines |
|---------|------|---------|----------|
| 0x230860 | dword | Free space result from HDAE5000_Calc_Disk_Space | HDAE5000_File_Format |
| 0x230864 | dword | Init sentinel (set to 0xFFFFFFFF) | HDAE5000_File_Save |
| 0x230868 | dword | File size in sectors (quotient) | HDAE5000_File_Save |
| 0x23086C | word | File descriptor offset | HDAE5000_File_Save |
| 0x23086E | word | Additional file offset | HDAE5000_File_Load |
| 0x230870 | word | File descriptor flag | HDAE5000_File_Save |
| 0x230872-0x230876 | 6 bytes | File save state variables | HDAE5000_File_Save |
| 0x23087E | byte | File type code 1 (mapped from 0x229DAD) | HDAE5000_File_Save |
| 0x230880 | byte | File type code 2 (mapped from 0x229DAE) | HDAE5000_File_Save |
| 0x230882 | byte | Terminator byte (from sector table) | HDAE5000_File_Format |
...
| Address | Size | Purpose |
|---------|------|---------|
| 0x23A06E | 16 bytes | Current tile buffer (for HDAE5000_FS_Read_FSB event handlers) |
...
| 0x23A0D2 | 200 bytes | Directory entry backup (5 slots x 40 bytes, used by HDAE5000_File_Delete) |
...
|----------|---------|------|--------|
| HDAE5000_FS_Init | 0x2870D6 | 3,711 bytes | Disassembled -- CHS entry UI with 10 event handlers |
| HDAE5000_FS_Read_FSB | 0x287F55 | 832 bytes | Disassembled -- display UI init (24 entries from template) |
| HDAE5000_FS_Write_FSB | 0x288295 | 5,072 bytes | Disassembled -- multi-sector serialization |
| HDAE5000_FS_Buffer_Setup | 0x289665 | 548 bytes | Disassembled |
| HDAE5000_FS_Scan_Directory | 0x289889 | 2,663 bytes | Disassembled -- iterates 20 buffer entries |
| HDAE5000_FS_Entry_Lookup | 0x28A2F0 | 739 bytes | Disassembled -- 9-block handler dispatch |
| HDAE5000_File_Operation | 0x28D6D1 | 938 bytes | Disassembled |
| HDAE5000_File_Save | 0x28DA7B | 381 bytes | Annotated -- type code mapping, sector calculation |
| HDAE5000_File_Load | 0x28DBF8 | 564 bytes | Annotated -- audio channel dispatch, filename slots |
| HDAE5000_File_Delete | 0x28DE2C | 579 bytes | Annotated -- directory backup, type 0x0D/0x0A dispatch |
| HDAE5000_File_Rename | 0x28E06F | 280 bytes | Annotated -- actually directory search/traversal |
| HDAE5000_File_Format | 0x28E187 | 772 bytes | Annotated -- error codes, sector validation |
| HDAE5000_Read_Table_Word | 0x28E417 | 52 bytes | Annotated -- 16-bit big-endian table reader |
| HDAE5000_Read_Table_Multi | 0x28E44B | 64 bytes | Annotated -- 24-bit big-endian table reader |
| HDAE5000_Calc_Disk_Space | 0x28E48B | 178 bytes | Annotated -- inline VarInt decode + encode |
| HDAE5000_Directory_Handler | 0x28F197 | 614 bytes | Disassembled |
| HDAE5000_VarInt_Encode | 0x28F36B | 82 bytes | Annotated -- MIDI-style 7-bit MSB-first encoding |
| HDAE5000_VarInt_Decode | 0x28F3BD | 64 bytes | Annotated -- MSB-first decode, max 5 bytes |
| HDAE5000_File_Read | 0x29AE24 | 123 bytes | Disassembled |
| HDAE5000_Cmd03_ReadFSB | 0x2959F6 | 838 bytes | Disassembled -- PPORT disk read |
| HDAE5000_Cmd04_SendFSB | 0x295D3C | 798 bytes | Disassembled -- PPORT send to PC |
| HDAE5000_Cmd05_RcvFSB | 0x29605A | 570 bytes | Disassembled -- PPORT receive from PC |
| HDAE5000_Cmd06_WriteFSB | 0x296294 | 150 bytes | Disassembled -- PPORT disk write |

### Routine Descriptions

**HDAE5000_FS_Init** (0x2870D6, 3,711 bytes) -- CHS address entry UI with 10 event handler sub-routines. Dispatches on parameter DE (indices 0-6 via jump table at 0x2E2D46) to configure cylinder/head spinbox widgets. Does NOT read from disk directly.

**HDAE5000_FS_Read_FSB** (0x287F55, 832 bytes) -- Display UI initialization. Copies the 21-byte ROM template to 24 entry slots in RAM at 0x22A2CA, formats entry numbers, and copies 16-byte display tile blocks. Does NOT read from disk -- the actual sector reads are performed by HDAE5000_Cmd03_ReadFSB.

**HDAE5000_FS_Write_FSB** (0x288295, 5,072 bytes) -- Serializes the in-RAM filesystem state to multiple disk sectors. Writes Sector 0 (header), Sector 1 (16 directory entries at 37 bytes each), Sector 2 (position), and Sectors 3-4 (navigation). Uses ROM handler tables at 0x2E2E7C-0x2E2EA4 (6 tables of function pointers).

**HDAE5000_FS_Scan_Directory** (0x289889, 2,663 bytes) -- Scans directory entries within a partition, iterating over FGB/FEB structures at the filesystem buffer base (0x22AA9C, up to 20 entries).

**HDAE5000_FS_Entry_Lookup** (0x28A2F0, 739 bytes) -- Looks up a specific file entry by dispatching 9 handler index bytes (at 0x22ABE8) through the function pointer table at ROM 0x2E1E14.

**HDAE5000_File_Save** (0x28DA7B, 381 bytes) -- Initializes save state, clears file descriptor fields (0x230438-0x230876), copies filename, computes sector count, and maps content types 0-4 to internal file type codes (0xF9, 0x02, 0xFC, 0x00, 0xFB).

**HDAE5000_File_Load** (0x28DBF8, 564 bytes) -- Loads file descriptors from directory. Three blocks: filename slot 1 (type 2, max 50 chars), filename slot 2 (type 3, max 40 chars), and audio settings (type 0x58 -- maps to 2/4/8/16 channels with 24/12/6/3 samples). Uses HDAE5000_File_Rename as a directory lookup.

**HDAE5000_File_Delete** (0x28DE2C, 579 bytes) -- Manages directory entries during deletion. Optionally backs up 5 entries (40 bytes each) before modifying. Dispatches on file type byte: 0x0D (raw copy), 0x0A (default name), other (concatenate if fits in 39 chars).

**HDAE5000_File_Rename** (0x28E06F, 280 bytes) -- Despite the name, this is a directory search/traversal function. Iterates partitions via HDAE5000_File_Format, searching for entries whose type code matches the requested operation type. Supports three DE modes: positive (iterate), -1 (error), -2 (use caller's stack arg).

**HDAE5000_File_Format** (0x28E187, 772 bytes) -- Formats a disk partition. Validates sector range (max 20,457), reads VarInt-encoded allocation data from sector table, checks for free (0xFF) and reserved (0x2F) sectors. Error codes: 0xFFFF-0xFFFA stored to (0x2307B6). Supports backup mode (flag bit 0).

**HDAE5000_Calc_Disk_Space** (0x28E48B, 178 bytes) -- Two sub-routines: (1) inline VarInt decode from sector table at 0x22B430 (up to 4 bytes per value), (2) VarInt encode value into buffer. Sub-routine 1 reads table[sector + 22] descriptors.

**HDAE5000_Read_Table_Word** (0x28E417) / **HDAE5000_Read_Table_Multi** (0x28E44B) -- Helper routines to read 16-bit and 24-bit big-endian values from the sector allocation table.

**HDAE5000_VarInt_Encode** (0x28F36B) / **HDAE5000_VarInt_Decode** (0x28F3BD) -- MIDI-style variable-length integer encoding. 7-bit chunks, MSB-first, bit 7 = continuation flag. Max 5 bytes (35 bits). Example: 389 encodes as [0x83, 0x05].
...
|---------------|---------|-----------|
| 03 | HDAE5000_Cmd03_ReadFSB (0x2959F6) | Read FSB sectors from hard disk into RAM |
| 04 | HDAE5000_Cmd04_SendFSB (0x295D3C) | Send FSB data from RAM to PC |
| 05 | HDAE5000_Cmd05_RcvFSB (0x29605A) | Receive FSB data from PC into RAM |
| 06 | HDAE5000_Cmd06_WriteFSB (0x296294) | Write FSB data from RAM to hard disk |
...
The firmware validates the superblock by checking the signature and magic as 32-bit register values.

⚠ **Unverified / possibly inconsistent:** this section (the disk-image byte layout, the
signature/magic values, and the "RONCO" string) is not derivable from anything in
`kn5000-roms-disasm` — no `.s` file mentions `55AA`, `AA55`, `F4F1F2F3` or `F3F2F1F4`, so it
must come from analysis of an actual disk image this pass did not have access to. Separately,
the signature is given two different forms in this page — `0x55AA55AA` in the table above
versus `0xAA55AA55` in the sentence this note replaces — which look like a byte-order
transcription slip of the kind the *magic* field's row already calls out explicitly ("LE;
firmware loads as..."). Neither form is confirmed against ROM or disk-image bytes here;
re-derive from the actual disk image before trusting either.
...
- HDAE5000_FS_Read_FSB event handlers: tile read/write, size query, action dispatch
...
- HDAE5000_FS_Scan_Directory buffer layout at 0x22AA9C (20 entries, complex stride pattern with offsets 0x100 and 0x114)
**Completed issues:**
- kn5000-q7xb: HDAE5000_FS_Read_FSB field-level annotation

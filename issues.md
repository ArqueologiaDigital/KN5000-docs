...
#### 🟡 LLVM converter: Fix unresolved JR/JRL branch labels (~1,500 .byte) {#issue-kn5000-ov84}

**ID:** `kn5000-ov84` | **Priority:** Medium | **Created:** 2026-02-22

About 1,500 JR/JRL relative branch instructions remain as .byte fallback because their target addresses are not found in ADDR_TO_LABEL. These are instructions in opcode range 0x60-0x7F where the converter correctly decodes the relative offset and computes the target address, but the label table doesn't contain an entry for that address. Possible causes: (1) targets are inside data sections that lack labels, (2) local labels generated by ASL that weren't captured, (3) computed addresses off by alignment. Need to investigate why these specific branch targets lack labels and fix the label table generation.
...
| `kn5000-4rk9` | LLVM converter: register-indirect addressing → semantic i... | 2026-02-22 |
| `kn5000-lzqh` | LLVM converter: batch small instruction conversions (+5,833) | 2026-02-22 |
| `kn5000-9w1z` | LLVM converter: Native remaining misc instructions (~2,00... | 2026-02-22 |

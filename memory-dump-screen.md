...
- An earlier static analysis in this investigation concluded that the DOWN button increments the address. That was WRONG - live observation shows UP increments, DOWN decrements. Independently, the firmware uses the same polarity test (bit 7 of the event code) in the memory dump and in the part-volume widget, so the button that RAISES a part volume on the home screen is the same one that RAISES the address here. Either way the address is printed on screen, so Step 5 is self-correcting.
- The emulator's reading at 0x3E0000 does NOT predict what a real unit shows. MAME overlays kn5000_subprogram_v142_compressed.rom into the custom_data region at offset 0x0E0000 (kn5000.cpp:1479); the genuine ic19 dump the project holds is 0xFF across that entire 128 KB. The emulator run proves the viewer reads the IC19 window through the CPU bus - it says nothing about what Felipe's chip contains, which is exactly the open question.

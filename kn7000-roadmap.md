...
| [Boot sequence](/boot-sequence/) | 🟢 boot header + reset vectors disassembled; runs in MAME **through hardware init, BSS setup, the MILK kernel, and into the self-loaded library ROM** (`0x4C03xxxx`) and deeper init (`0x4854xxxx`). Next it parks in a panel/scheduler wait loop needing CPU interrupts | implement MN10300 interrupts so the scheduler runs its tasks (and draws the UI) |
...
| MN10300 CPU core | 🟢 **~99.94% of instructions implemented, VERIFIED in MAME** (boot I/O trace byte-identical to the Python interpreter). **Interrupt mechanism fully reverse-engineered + specced** (`kn7000_mame/notes/interrupt-mechanism.md`): the self-loaded library ROM provides the dispatcher (IAGR `0x34000100` → handler table `0x50380B64`) + `rti` (`0x4C03DE24`); the core needs `take_irq`/`rti`/`check_irq`/`execute_set_input` + `PSW.IM`, an INTC device, and a `0x32000000` timer | implement interrupts (spec ready) → scheduler ticks → UI draws + panel/MIDI RX |

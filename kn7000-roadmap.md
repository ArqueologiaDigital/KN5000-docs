...
| Buildable disassembly → **100% byte-perfect** ROM rebuild | 🟡 byte-exact; organised by the **1,892 functions**; **38 functions converted to real re-assemblable MN10300 assembly with symbol-resolved calls** (`call GetLinkView`, not a raw address) — the encoder + `kn7asm` assemble it back to exact bytes | grow the `CONVERT` set; auto-name internal helpers so more calls resolve |
